...
(() => {
  ...
  function renderPeopleView() {
    ...
    adminPeopleView.innerHTML = `<div class="admin-people-shell">
      ...
      <div class="admin-body"><div class="admin-list-panel"><div class="admin-toolbar"><input id="adminSearch" type="search" value="${esc(adminSearch)}" placeholder="Search everyone across all stores..."><select id="adminLocationFilter"><option value="">All locations</option>${locations.map(location => `<option>${esc(location)}</option>`).join("")}</select><select id="adminDepartmentFilter"><option value="">All departments</option>${departments.map(department => `<option>${esc(department)}</option>`).join("")}</select><select id="adminSourceFilter"><option value="">All records</option><option value="source">Website / imported</option><option value="manual">Manual review</option></select><button class="admin-clear" type="button" data-action="clear-filters">Clear filters</button></div>
        <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Photo</th><th>Name ↑</th><th>Role ↕</th><th>Location ↕</th><th>Department ↕</th><th>Phone ↕</th><th>Ext ↕</th><th>⋮</th></tr></thead><tbody>${pageRows.length ? pageRows.map(person => `<tr class="${keyFor(person) === selectedKey ? "selected" : ""}" data-key="${esc(keyFor(person))}"><td>${avatar(person)}</td><td class="admin-name">${esc(person.name)}</td><td title="${esc(person.title)}">${esc(person.title)}</td><td>${esc(person.location)}</td><td>${esc(person.department)}</td><td>${esc(person.phone || "—")}</td><td>${esc(person.extension || "—")}</td><td><button class="admin-kebab" type="button" data-action="edit" data-key="${esc(keyFor(person))}" aria-label="Edit ${esc(person.name)}">⋮</button></td></tr>`).join("") : `<tr><td colspan="8" class="admin-empty">No staff records match the current filters.</td></tr>`}</tbody></table></div>${pagination(shown.length,totalPages)}</div><aside class="admin-inspector">${inspector(selected)}</aside></div></div>`;
  ...
  function openAdmin(view = "directory") { activeView = view; adminLocationFilter = ""; selectedKey = ""; page = 1; adminModal.hidden = false; document.body.classList.add("admin-open"); renderAdmin(); }
